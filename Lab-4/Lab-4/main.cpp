...
DWORD WINAPI ThreadProc( CONST LPVOID lpParam ) {
	...
	for ( int i = startRow; i <= endRow; i++ )
	{
		for ( int j = 0; j < width; j++ )
		{
			...
			clock_t time = std::clock() - _startTime;
			*data->outputStream << time << std::endl;
		}
	}
	ExitThread( 0 );
}
...
void Blur(char* inputImgName, char* outputImageName, int threadCount, int coresCount, std::vector<ThreadPriority> prioritiesForThreads ) {
	...
	for ( int i = 0; i < threadCount; i++ )
	{
		...
		int threadNum = i + 1;
		outputFilesStreams[i] = std::ofstream( "t_" + std::to_string( threadNum ) + ".txt" );

		threadData[i] = { &inputImage, &outputImage, startRow, endRow, BLUR_RANGE_SIZE, threadNum, &outputFilesStreams[i] };
